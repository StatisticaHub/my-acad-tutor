  ...
  return (
    <section className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-[2rem] border border-neutral-200 bg-white p-6 shadow-sm md:p-8">
        ...
        <p className="mt-5 text-base leading-8 text-neutral-700">{body}</p>
...
function ShapeSelector({
  ...
  return (
    <div>
      <p className="text-sm font-black text-neutral-700">Relationship shape</p>
    ...
    <svg viewBox="0 0 520 260" className="h-auto w-full">
      ...
      {data.map((d, index) => (
        <circle
          ...
          fill={index === data.length - 1 ? "#8b1116" : "#ffffff"}
          opacity="0.9"
        />
      ))}
...
function ScatterChecklistVisual() {
  return (
    <svg viewBox="0 0 760 420" className="h-auto w-full">
      ...
      <line x1="105" x2="660" y1="300" y2="300" stroke="#111111" strokeWidth="4" />
      <line x1="105" x2="105" y1="95" y2="300" stroke="#111111" strokeWidth="4" />

      {Array.from({ length: 38 }).map((_, i) => {
        const x = 130 + i * 13;
        const y = 270 - i * 3.7 + 18 * Math.sin(i * 1.4);
        return <circle key={i} cx={x} cy={y} r="6" fill="#111111" opacity="0.7" />;
      })}

      <circle cx="610" cy="105" r="11" fill="#8b1116" />
      ...
      <text x="545" y="370" fontSize="18" fontWeight="900" fill="#8b1116">Outliers</text>
    </svg>
  );
}
